...
class StudentAI():
    ...
    def get_move(self,move):
        ...
        new_root = self.root.make_move(move)
        if new_root:
            self.root = new_root
...
class MCTS:
    ...
    def select(self, node: Node) -> Tuple[Node, Board]:
    ...
    def expand(self, node: Node, board: Board) -> Tuple[Node, Board]:
        """ Expand children of selected node 
            @param:  node - Node selected to expand
            @return: Newly created expanded child Node
        """
        if board.is_win(3 - node.get_color()) != 0:                    # Node is terminal, no expansion possible
            return node, board

        move = random.choice(node.get_untried_moves())                  # Pick a random move to create expanded node
        board.make_move(move, node.get_color())                    # Perform move
        moves = board.get_all_possible_moves(3 - node.get_color()) # Get all possible moves for child node
        new_node = node.add_child(move, moves)                          # Create child node
        return new_node, board

    def simulate(self, node: Node, board: Board) -> int:
        ...
        if result == 0:
            result = board.is_win(3 - color) # Check if opponent won

        while result == 0:                # While no outcome has been determined yet (no one has won)
            moves = board.get_all_possible_moves(color)    # Get all possible moves according to color player
            if not moves or not any(moves):
                result = 3 - color
                break
            move = random.choice(random.choice(moves))          # Randomly select a move
            board.make_move(move, color)                   # Perform move
            result = board.is_win(color)
            color = 3 - color                                   # Swap color, 1 if color was 2, 2 if color was 1
        # result = board.is_win(3 - color)                       # -1 for tie, 1 for black win, 2 for white win
        return 1 if node.get_color() == result or result == -1 else 0 # 1 if winner is same as node color
                                                                                # or if tie, otherwise 0
        # color = node.get_color()                                # Tracks the color of the current move
        # while board.is_win(3 - color) == 0:                # While no outcome has been determined yet
        #     moves = board.get_all_possible_moves(color)    # Get all possible moves according to color player
        #     move = random.choice(random.choice(moves))          # Randomly select a move
        #     board.make_move(move, color)                   # Perform move
        #     color = 3 - color                                   # Swap color, 1 if color was 2, 2 if color was 1
        # result = board.is_win(color)                       # -1 for tie, 1 for black win, 2 for white win
        # return 1 if node.get_color() == result or result == -1 else 0 # 1 if winner is same as node color
        #                                                                         # or if tie, otherwise 0
    ...
    def search(self, node: Node) -> Move:
        ...
        for _ in range(self.num_simulations):                            # Runs for specified number of iterations
            leaf, leaf_board = self.select(node)                         # Select a leaf node to perform expansion
            new_child, new_board = self.expand(leaf, leaf_board)         # Create child node from leaf node                   
            reward = self.simulate(new_child, new_board)                 #  Simulate game from the child node and retrieve the outcome                   
...
### Version 2: Tree saving with Board undo ###
# class StudentAI():
...
### Version 1: No tree saving with Board undo ###
# class StudentAI():
